...
### 2.2 Control 화면 (진행자 / 관리자)
- **접속 주소**: `https://<PROJECT_ID>.web.app/control.html` (로컬: `http://localhost:5173/control.html`)
- **로그인**: 승인된 Google 계정으로 로그인해야 제어 화면에 진입합니다.
- **모드 전환 (State Switching)**: 상단 헤더의 모드 스위처를 통해 언제든 전환할 수 있습니다.

#### ① 🎬 프리젠테이션 모드 (기본값)
- **목적**: 전례 진행 중 실수 없는 안전한 송출 및 제어에만 집중하는 화면입니다.
- **특징**:
  - 미사/슬라이드 추가·삭제 및 순서 변경, 역할 변경 등의 관리 도구는 숨겨져 오작동을 방지합니다.
  - 진행 중 오타 수정을 위해 **본문 텍스트는 클릭하여 즉시 인라인 수정**할 수 있으며, 수정 내용이 프로젝터 화면에 실시간 자동 반영됩니다.
  - 슬라이드 목록 클릭 또는 단축키(`Space`, `←`, `→`) 입력 시 프로젝터(Display) 화면으로 실시간 송출됩니다.
  - `Display 끄기`(Blackout), `Display 멈추기`(Freeze) 등의 송출 제어 도구가 활성화됩니다.

#### ② 🛠️ 수정(편집) 모드 (Admin Mode)
- **목적**: 미사 전례 내용, 슬라이드 순서, 본문 및 역할을 자유롭게 작성/수정하는 화면입니다.
- **특징**:
  - **프로젝터 송출 차단**: 슬라이드를 탐색하거나 텍스트를 편집해도 **현재 프로젝터(Display) 화면에 전혀 영향을 주지 않고 안전하게 데이터만 저장**됩니다.
  - **편집 도구 활성화**: 미사 생성/수정/삭제, 슬라이드 추가/삭제/드래그앤드롭 순서 변경, 실시간 본문/제목 텍스트 인라인 수정, 역할(➕, ◎, ○, ●) 변경, 정렬 및 굵기 설정이 활성화됩니다.

#### 주요 제어 기능 및 단축키 (프리젠테이션 모드)
| 기능 | 단축키 / 방법 | 설명 |
| :--- | :--- | :--- |
...
| **모드 전환** | 상단 `프리젠테이션` / `수정(편집)` | 제어 모드와 데이터 수정 모드 즉시 전환 |
